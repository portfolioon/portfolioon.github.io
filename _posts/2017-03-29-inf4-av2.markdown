...
images:
  - {
    ...
    display: "full"  # Add this to make it full width
  }
  - {
...
{% for image in page.images %}
  <div class="project-image {% if image.display == 'full' %}project-image-full{% else %}project-image-third{% endif %}">
    {% if image.url contains '.webm' or image.url contains '.mp4' %}
      <video class="proj-img" autoplay loop muted playsinline>
        <source src="{{ image.url }}" type="video/{% if image.url contains '.webm' %}webm{% else %}mp4{% endif %}">
      </video>
    {% else %}
      <img src="{{ image.url }}" class="img-responsive proj-img">
    {% endif %}
    {% if image.caption %}
